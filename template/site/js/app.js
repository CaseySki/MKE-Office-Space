/* Commercial Office Space — Site JS */

/* ── Config (loaded from config.js) ── */
const CONFIG = {
  SHOW_LEASED: SITE_CONFIG.SHOW_LEASED !== undefined ? SITE_CONFIG.SHOW_LEASED : true,
...
function initMap(buildings, suites) {
  ...
  pinData.forEach((pin, idx) => {
    ...
    const leaseColor = SITE_CONFIG.BRAND_COLOR || "#CF152D";
...
function initSuiteSearch(buildings, suites) {
  ...
  const PAGE_SIZE = SITE_CONFIG.SUITES_PER_PAGE || 10;
  let currentPage = 1;
...
function getFavorites() {
  try { return JSON.parse(localStorage.getItem("cre_favorites") || "[]"); }
  catch { return []; }
}

function saveFavorites(favs) {
  localStorage.setItem("cre_favorites", JSON.stringify(favs));
}
...
function initInquiryForm(buildingName) {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const subject = encodeURIComponent(`${SITE_CONFIG.INQUIRY_SUBJECT_PREFIX} — ${data.building || SITE_CONFIG.COMPANY_NAME}`);
    const body = encodeURIComponent(
      `Name: ${data.name}\nCompany: ${data.company}\nEmail: ${data.email}\nPhone: ${data.phone || "N/A"}\n\nBuilding: ${data.building || "N/A"}\n\nMessage:\n${data.message || "N/A"}`
    );
    window.location.href = `mailto:${SITE_CONFIG.INQUIRY_EMAILS}?subject=${subject}&body=${body}`;
...
function loadCompareList() {
  try { compareList = JSON.parse(sessionStorage.getItem("cre_compare") || "[]"); }
  catch { compareList = []; }
}

function saveCompareList() {
  sessionStorage.setItem("cre_compare", JSON.stringify(compareList));
}
...
function getRecentlyViewed() {
  try { return JSON.parse(localStorage.getItem("cre_recent") || "[]"); }
  catch { return []; }
}
...
function saveRecentView(building) {
  ...
  const maxRecent = SITE_CONFIG.MAX_RECENT_VIEWS || 4;
  if (recent.length > maxRecent) recent = recent.slice(0, maxRecent);
  localStorage.setItem("cre_recent", JSON.stringify(recent));
}
